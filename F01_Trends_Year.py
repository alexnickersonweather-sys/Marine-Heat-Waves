...
mg = 1*g # year threshhold for means in years
my = 1*y # year threshhold for means in years

for i, stat in stat_all.iterrows():
    file = "Data/" + stat["Type"].strip() + "/Climo/" + stat["Reg"].strip() + '_Year.nc'
    ...
    for d in dep:
        ...
        if ng >= mg and ny >= my:
            slope, yint, dof, t95, ci95 = trend(yr,df[d],"y")
        elif ng < mg and ny >= my:
            slope, yint, _, _, _ = trend(yr,df[d],"y")
            dof = np.nan
            t95 = np.nan
            ci95 = np.nan
        else:
...
trends.to_csv('Stats/Trends_Year.txt',index=False,sep="|")
